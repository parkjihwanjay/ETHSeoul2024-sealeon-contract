...
let HelloNearV2 = (_dec = NearBindgen({}), _dec2 = view(), _dec3 = call({}), _dec(_class = (_class2 = class HelloNearV2 {
  greeting = "HelloV2";
...
function set_greeting() {
  const _state = HelloNearV2._getState();
  if (!_state && HelloNearV2._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = HelloNearV2._create();
  if (_state) {
    HelloNearV2._reconstruct(_contract, _state);
  }
  const _args = HelloNearV2._getArgs();
  const _result = _contract.set_greeting(_args);
  HelloNearV2._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(HelloNearV2._serialize(_result, true));
}
...
//# sourceMappingURL=hello_nearv2.js.map
